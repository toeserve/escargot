...
class UserContact(WithFrontData):
	...
	yahoo_contact_id = Col(sa.String, nullable = True) # TODO: For yahoo, like group_id; need Unique(user_id, yahoo_contact_id)
	birthdate = Col(sa.DateTime, nullable = True)
	anniversary = Col(sa.DateTime, nullable = True)
	notes = Col(sa.String)
	display_name = Col(sa.String)
	...
	groups = Col(JSONType)
	is_messenger_user = Col(sa.Boolean, default = False) # TODO: rename since it's a more general concept
...
# TODO: This could replace `UserContact.lists`
# `RL` is checked by looking up the users in reverse manner and checking if `is_following`
#class ???(Base):
#	owner_user_id = Col(sa.Integer, sa.ForeignKey('t_user.id'), primary_key = True)
#	other_user_id = Col(sa.Integer, sa.ForeignKey('t_user.id'), primary_key = True)
#	is_following = Col(sa.Boolean)
#	is_blocked = Col(sa.Boolean)
#	is_allowed = Col(sa.Boolean)
